...
def get_dataset(opts, tr_ds_name=None):
    ...
    if name == 'cityscapes':
        ...
        import inspect
        acdc_sig = inspect.signature(ACDC)
        kw = {}
        if getattr(opts, "ACDC_test_class", None):
            if 'test_class' in acdc_sig.parameters:
                kw['test_class'] = opts.ACDC_test_class
            elif 'condition' in acdc_sig.parameters:
                kw['condition'] = opts.ACDC_test_class
        tst_dst = ACDC(root=opts.data_root_acdc, split='val', transform=val_transform, **kw)
        return train_dst, val_dst, tst_dst
...
def validate(opts, model, loader, device, metrics,
    ...
    # ... existing output directory setup
...
def apply_mode_presets(opts):
    ...
    if opts.test_only:
        # evaluation-friendly defaults (can be overridden by CLI)
        if opts.batch_size == 16:  # Only if using default
            opts.batch_size = 1
        if opts.test_batch_size == 4:  # Only if using default
            opts.test_batch_size = 1
        if opts.val_batch_size == 4:  # Only if using default
            ...
            for step in range(max_len):
                ...
                with autocast():
                    ...
                    if getattr(opts, "disable_ws_tas", False) or (opts.ws_weight == 0 and opts.tes_weight == 0):
                        # Prevent any chance of gradients through aux heads
                    ...
                    if is_acdc and (opts.ws_weight > 0 or opts.tes_weight > 0):
                        loss_ws_ce  = criterion(weather_preds, weather_ids) if opts.ws_weight > 0 else torch.tensor(0.0, device=images.device)
                        loss_tes_ce = criterion(time_preds,   time_ids)     if opts.tes_weight > 0 else torch.tensor(0.0, device=images.device)
                ...
                if opts.check_backbone_grads and (cur_itrs % opts.log_heads_every == 0):
                    def grad_norm(module):
                        ...
                        return (total ** 0.5)
                    if hasattr(model, "backbone"):
                        gn = grad_norm(model.backbone)
                        print(f"[grads] iter {cur_itrs} | domain={domain_name} | backbone grad-norm={gn:.6f}")
                ...
                if (cur_itrs) % opts.log_heads_every == 0:
                    if getattr(opts, "disable_ws_tas", False) or (opts.ws_weight == 0 and opts.tes_weight == 0):
                        ...
                        acc_ws_txt  = f"{ws_acc*100:.1f}%" if is_acdc and ws_acc is not None else "N/A"
                        acc_tes_txt = f"{tes_acc*100:.1f}%" if is_acdc and tes_acc is not None else "N/A"
                        print(f"Epoch {cur_epochs}, Itrs {cur_itrs}/{opts.total_itrs}, "
