# main_mask2former_WAS_TAS_ON_Disable_WAS_TAS.py
...
def main(ACDC_test_class=None, n_itrs=90000):
    ...
    print(f"[ABLATION] ws_weight={opts.ws_weight} | tes_weight={opts.tes_weight} "
          f"| seg_scale_ACDC=1.0 | seg_scale_CS=1.0")
    ...
    while cur_itrs < opts.total_itrs:
        ...
        if opts.mode == 0:
            # ===========================================================
            # DATASET-WISE ALTERNATION:
            # First run through ALL Cityscapes batches,
            # then run through ALL ACDC batches.
            # No encoder freezing: backbone always trainable.
            # ===========================================================

            # ------------- Full Cityscapes pass -------------
            print(f"[EPOCH {cur_epochs}] Cityscapes pass")
            for step_cs, (images, labels, names, weather_ids, time_ids, _) in enumerate(train_loader_cs):
                ...
                # No encoder freezing in ablation: everything trainable
                ...
                with autocast():
                    ...
                    use_heads = (opts.ws_weight > 0 or opts.tes_weight > 0) and \
                                not getattr(opts, "disable_ws_tas", False)
                    if use_heads:
                        loss_ws_ce = criterion(weather_preds, weather_ids) if opts.ws_weight > 0 else \
                            torch.tensor(0.0, device=images.device)
                        loss_tes_ce = criterion(time_preds, time_ids) if opts.tes_weight > 0 else \
                            torch.tensor(0.0, device=images.device)
                    ...
                    total_loss = loss_seg + loss_weather + loss_time
                ...
                if cur_itrs % opts.val_interval == 0:
                    ck_latest = 'checkpoints/main_mask2former_whole_dataset_alternation_no_batchwise_no_encoder_freezing_no_was_tas_latest.pth'
                    ck_best = 'checkpoints/main_mask2former_whole_dataset_alternation_no_batchwise_no_encoder_freezing_no_was_tas_best.pth'
                    save_ckpt(ck_latest)
                    print("validation...")
                    model.eval()
            ...
            # ------------- Full ACDC pass -------------
            print(f"[EPOCH {cur_epochs}] ACDC pass")
