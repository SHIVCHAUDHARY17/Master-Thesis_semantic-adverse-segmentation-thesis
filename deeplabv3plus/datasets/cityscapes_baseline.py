...
class Cityscapes(data.Dataset):
    ...
    def __init__(
        ...
        transform=None,
        indices: Optional[List[int]] = None,   # NEW: optional sub-selection for 85/15, etc.
        ...
        for city in sorted(os.listdir(self.images_dir)):
            ...
            for fname in sorted(os.listdir(city_img_dir)):
                ...
                tgt_name = fname.replace('_leftImg8bit.png', f'_{self._get_target_suffix(self.mode, self.target_type)}')
    ...
    def __getitem__(self, index):
        ...
        name = os.path.basename(self.targets[index])

        if self.transform:
            image, label = self.transform(image, label)

        # convert labelIds -> trainIds
        label_np = np.array(label, dtype=np.int64)
        label_copy = 255 * np.ones(label_np.shape, dtype=np.int64)
        ...
        return (
            image,
            torch.from_numpy(label_copy).long(),
            name,
...
def build_cityscapes_train85_val15_datasets(
    ...
    You choose transforms:
      - train_transform (with augmentations) is used for anything in training
      - val_transform (no heavy aug) is used for the held-out 15%
    """
    # Full 'train' set twice: once with train aug, once with val transform for the held-out indices
