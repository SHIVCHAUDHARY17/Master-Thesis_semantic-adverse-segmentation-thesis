...
# Task:    Segmentation ONLY (WAS/TAS fully removed)
...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(loader)):
            # Loader may yield 6-tuple; we only need images, labels, names
            # Original order: images, labels, names, weather_ids, time_ids, data_domain
            images, labels, names, *_ = batch
...
def main():
    ...
    print(f"[cfg] Seeds: torch={opts.random_seed} numpy={opts.random_seed} random={opts.random_seed}")
    ...
    if MODE == 0:
        ...
        print(f"[split] ACDC       TRAIN: train85={len(acdc_train)} | val15={len(acdc_val)}")
        ...
        print(f"[eval] Dataset={dataset_flag} | size={len(tst)} | per-condition={opts.ACDC_test_class}")
    ...
    # >>> Changed here: checkpoint names include REMOVED_WAS_TAS <<<
    def save_ckpt(tag):
        path = f"checkpoints/{tag}_REMOVED_WAS_TAS_{opts.model}_mix_os{opts.output_stride}.pth"
        torch.save({
            "cur_itrs": cur_itrs,
    ...
    print("[train] BEGIN training (SEG-ONLY)")
    ...
    while cur_itrs < opts.total_itrs:
        ...
        for i in range(max_iter):
            ...
            if i % 2 == 0:
                # (images, labels, names, weather_ids, time_ids, data_domain)
                images, labels, names, *_ = next(it_acdc)
                _toggle_low_level(model, True)
                if cur_itrs < 5:  # print first few toggles verbosely
                    print(f"[train] itr={cur_itrs+1} | ACDC batch | low_level_features.trainable=True")
            else:
                images, labels, names, *_ = next(it_cs)
                _toggle_low_level(model, False)
                if cur_itrs < 5:
                    print(f"[train] itr={cur_itrs+1} | CS   batch | low_level_features.trainable=False")
            ...
            # Model may return list/tuple; we only use seg logits
            outputs = out[0] if isinstance(out, (list, tuple)) and len(out) >= 1 else out
            ...
            if cur_itrs % opts.print_interval == 0:
                ...
                print(f"[itrs] {cur_itrs}/{opts.total_itrs} | loss(seg)={avg_loss:.6f} "
                      f"| lrs(backbone,cls)={lrs} | epoch={cur_epochs}")
                interval_loss = 0.0
