    ...
    def get_images_paths(self,image_dir, percentage=100, nTrain_samples=None, params=None):
        ...
        for filename in glob.iglob(image_dir + '/**/RGB_Shaky_Fr_*.png', recursive=True):
            if filename.find("RGB_Shaky_Fr_") != -1:
                # ------------------------------
                img = filename.replace(self.root + '/', '')
                temp = img.split('/')
                seq_name, frame_name = temp[0], temp[3]
                xml_path = os.path.join(self.root, seq_name, 'Annotations', 'Textual', 'TextualInfo.xml')
...
# shuffle train/valid/test data
        ...
        if split =="test" and FLAG:
            print("".join("{}\t{}\n".format(x.replace(self.root + '/', ''),
                                            y.replace(self.root + '/', '')) for x, y in zip(self.images, self.targets)))
    ...
    def __getitem__(self, index):
        filename = self.images[index]   # <== ADD THIS
...
# daytime = 0, night= 1
        ...
        return (
            ...
            name.split("/")[-1],  # Get just the image filename, e.g., 'RGB_Shaky_Fr_012.png'
            torch.tensor(weather_id).long(),
            torch.tensor(time_id).long(),
            torch.tensor(data_domain).long()
        )
