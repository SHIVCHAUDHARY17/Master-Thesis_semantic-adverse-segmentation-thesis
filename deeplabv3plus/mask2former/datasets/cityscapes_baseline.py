...
class Cityscapes(data.Dataset):
    ...
    def __init__(self, root, split='train', mode='fine', target_type='semantic', transform=None):
        ...
        self.class_conversion_dict = {
            # Map original Cityscapes IDs to 0-18 train IDs (19 classes)
            7: 0,    # road
            8: 1,    # sidewalk
            11: 2,   # building
            12: 3,   # wall
        ...
        for filename in glob.iglob(root + '/leftImg8bit/**/*.png', recursive=True):
            if filename.find("/train/") != -1 and split !="test":
                self.images.append(filename)
                filename = filename.replace("/leftImg8bit/", "/gtFine/")
                target_name = '{}_{}'.format(filename.split('_leftImg8bit')[0],
                                             self._get_target_suffix(self.mode, self.target_type))
                self.targets.append(target_name)
            if filename.find("/val/") != -1 and split =="test":
                self.images.append(filename)
                filename = filename.replace("/leftImg8bit/", "/gtFine/")
                target_name = '{}_{}'.format(filename.split('_leftImg8bit')[0],
                                             self._get_target_suffix(self.mode, self.target_type))
    ...
    def __getitem__(self, index):
        ...
        label = Image.open(self.targets[index])#.convert('RGB')
