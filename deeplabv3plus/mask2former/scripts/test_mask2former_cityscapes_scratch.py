...
def main(ACDC_test_class=None, n_itrs=30000, MODE=12345):
    ...
    MODE = 1     # train on cityscapes only   <-- use this
    ...
    if opts.test_only:
        ...
        if MODE == 0:
            # CS + AWSS training, ACDC & CS validation (unchanged)
            ...
            train_loader_cs = data.DataLoader(train_dst_cs, batch_size=opts.batch_size,
                                              shuffle=True, num_workers=0, drop_last=True)

            val_loader_cs = data.DataLoader(val_dst_cs, batch_size=opts.val_batch_size,
    ...
    while cur_itrs < opts.total_itrs:
        ...
        if MODE == 0:
            ...
            for images, labels, _, _, _, _ in train_loader_cs:
                cur_itrs += 1
                images = images.to(device, dtype=torch.float32)
                labels = labels.to(device, dtype=torch.long)
                ...
                # Your model returns (seg_logits, weather_preds, time_preds)
...
if __name__ == '__main__':
    ...
    MODE = 1  # train on Cityscapes only (ablation)
    # MODE = 11  # test on cityscapes
    # MODE = 21  # test on acdc
